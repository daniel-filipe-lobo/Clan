	...
	internal class Clash : IClash
	{
		...
		public async Task ProcessAsync(AuthenticationTokeReference authenticationTokeReference, string? fileName)
		{
			try
			{
				...
				var clanDetail = await GetAndDeserializeAsync<ClanDetail>($"https://api.clashofclans.com/v1/clans/{clanTagEncoded}", authenticationToke);
				if (clanDetail == null)
				{
					return;
				}
				...
				var leagueGroup = await GetAndDeserializeAsync<LeagueGroup>($"https://api.clashofclans.com/v1/clans/{clanTagEncoded}/currentwar/leaguegroup", authenticationToke);
				if (leagueGroup == null)
				{
					return;
				}
				...
				{
					var modelWar = await GetAndDeserializeAsync<ConsoleClan.Models.War>($"https://api.clashofclans.com/v1/clans/{clanTagEncoded}/currentwar", authenticationToke);
				...
				else
				{
					foreach (var round in leagueGroup.rounds.EmptyIfNull())
					{
						foreach (var warTag in round.warTags.EmptyIfNull())
						{
							...
							var modelWar = await GetAndDeserializeAsync<ModelWar>($"https://api.clashofclans.com/v1/clanwarleagues/wars/{warTagEncoded}", authenticationToke);
							if (modelWar == null)
							{
								continue;
							}
		...
		private async Task UpsertWarLeagueAsync(DataWar war, LeagueGroup leagueGroup)
		{
		...
		private async Task<DataWar> WarUpsertAsync(string? warTag, ModelWar modelWar)
		{
			DataWar? war;
		...
		private async Task<bool> TryReadWarAsync(string? warTag, ModelWar? modelWar, Dictionary<string, MemberScore> memberScores, LeagueGroup? leagueGroup)
		{
		...
		private async Task UpsertWarPlayerAsync(DataWar war, Member member)
		{
		...
		private async Task UpsertAttackAsync(ModelAttack modelAtack, DataWar war, Member member, Member enemyMember)
		{
